...
def _build_paper_guide_grounding_rules(
    *,
    ...
    lines = [
        ...
        "- Treat every paper-specific mechanism, result, number, comparison, and limitation as its own evidence-bearing claim; put its support marker on that exact sentence or omit it.",
        "- Keep neighboring modalities separate (for example, single-photon versus single-pixel imaging). A neighboring modality may be labeled as background, but its evidence cannot prove a target-modality claim.",
        "- Do not infer that the paper or experiment lacks validation merely because the current snippet does not show it; say only that the currently cited evidence does not directly state it.",
