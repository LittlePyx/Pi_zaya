              ...
              card_view: {
                ...
                sections: [
                  ...
                  },
                  {
                    ...
                    text: 'The two-step source passage directly explains the ray-tracing and wave-propagation mechanism summarized in the answer.',
                    kind: 'support',
                    hint: '',
                    tone: '',
                  },
...
const cjkAdjacentStrongMessages: Message[] = [
  {
    id: 9221,
    role: 'assistant',
    content: '三个工作的共同起点是**快照压缩成像（CASSI）**的物理原理，深度学习能够**显著降低系统复杂度**，并进一步发展到**三维场景表示**和动态重建。',
    rendered_body: '三个工作的共同起点是**快照压缩成像（CASSI）**的物理原理，深度学习能够**显著降低系统复杂度**，并进一步发展到**三维场景表示**和动态重建。',
    copy_text: '三个工作的共同起点是快照压缩成像（CASSI）的物理原理，深度学习能够显著降低系统复杂度，并进一步发展到三维场景表示和动态重建。',
    copy_markdown: '三个工作的共同起点是**快照压缩成像（CASSI）**的物理原理，深度学习能够**显著降低系统复杂度**，并进一步发展到**三维场景表示**和动态重建。',
    created_at: Date.now(),
  },
]
...
const agentTraceReferenceActionMessages: Message[] = [
  {
    ...
    meta: {
      agent_trace: {
        ...
        research_run: {
          run_id: 'rr_fixture_reference_followup',
          status: 'verified',
          source_policy: 'local_only',
          query_scope: 'current_paper',
          question: 'Which upstream reference should I read?',
          subtasks: [
            {
              goal: 'Retrieve local evidence from the indexed library.',
              tool: 'retrieve_evidence',
              status: 'done',
              observation: '1 usable local evidence hit.',
            },
            {
              goal: 'Resolve upstream references from citing-paper evidence.',
              tool: 'retrieve_references',
              status: 'done',
              observation: 'Resolved bibliography entry [24].',
            },
            {
              goal: 'Verify local citation support and disclose non-local context.',
              tool: 'verify_answer_citations',
              status: 'done',
              observation: 'Verification status: needs_review.',
            },
          ],
          evidence_matrix: [
            {
              paper: 'Fast hyperspectral single-pixel imaging via frequency-division multiplexed illumination',
              source_name: READER_REGRESSION_SOURCE_NAME,
              source_path: READER_REGRESSION_SOURCE_PATH,
              method: 'Frequency-division multiplexed illumination for hyperspectral single-pixel imaging.',
              dataset_or_experiment: 'Fixture Paper / Related Work',
              key_result: 'Identified as the upstream prior work most relevant to the question.',
              limitation: 'Not identified in retrieved evidence.',
              evidence_quote: 'The current paper cites frequency-division multiplexed illumination as prior work [24].',
              citation: '[24]',
              heading_path: 'Fixture Paper / Related Work',
        ...
        verification: {
          ...
          claims: [
            {
              index: 1,
              claim_text: 'For upstream reading, start with Fast hyperspectral single-pixel imaging.',
            ...
            {
              index: 2,
              claim_text: 'The prior work fully solves every downstream limitation.',
              citation_present: true,
              has_citation: true,
              matched_evidence_count: 0,
              matched_sources: [],
              supported: false,
              unsupported_reason: 'missing_evidence_overlap',
            },
          ],
        },
      },
...
const agentSourceSummaryLocalMessages: Message[] = [
  {
    ...
    meta: {
      ...
      agent_source_summary: {
        kind: 'unknown',
        label_key: 'agent_trace_source_fallback',
        label: 'Source',
        detail: '',
        confidence: 'high',
