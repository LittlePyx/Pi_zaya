...
export const RESEARCH_QA_CASES: ResearchQaCase[] = [
  ...
  makeCase(10, {
    ...
    answerMarkdown: [
      ...
      '',
      '所以不是三篇都在做同一件事：CASSI 是压缩光谱成像装置，SCINeRF 是 SCI 到 NeRF，SCIGS 是 SCI 到显式 Gaussian primitives。要追上游 SCI 背景，可以点开 SCINeRF Related Work 里的参考 [R7](#qa-cassi-lineage-b7)。',
    ].join('\n'),
    citeDetails: [
      systemA({
        num: 1,
      ...
      systemB({
        num: 7,
        anchor: 'qa-cassi-lineage-b7',
        citingDoc: scinerf,
        headingPath: 'SCINeRF / 2. Related Work',
        context: 'SCINeRF cites earlier snapshot compressive imaging work when motivating SCI reconstruction.',
        ...
        raw: '[7] Coded aperture snapshot compressive imaging: theory and systems.',
        title: 'Coded aperture snapshot compressive imaging',
        authors: 'SCI community',
        venue: 'Optics and computational imaging literature',
        year: '2014',
        blockId: 'scinerf-rw-sci',
        anchorId: 'scinerf-rw-sci-a',
      }),
    ],
    refs: [
      refHit({
  ...
  makeCase(12, {
    ...
    answerMarkdown: [
      ...
      '',
      '继续追来源时，可以点开 PIDL 文中关于 SPAD noise 的上游参考 [R6](#qa-single-photon-pair-b6)：它解释这些噪声项为什么要进入模型。',
    ].join('\n'),
    citeDetails: [
      systemA({
        num: 1,
      ...
      systemB({
        num: 6,
        anchor: 'qa-single-photon-pair-b6',
        citingDoc: pidlSinglePhoton,
        headingPath: 'Physics-informed single-photon imaging / References',
        context: 'The PIDL paper points to upstream work on SPAD noise and single-photon detector modeling.',
        answerClaim: 'SPAD noise 是 PIDL 方法需要显式建模的上游基础。',
        upstreamWorkRole: '这类文献定义或测量 SPAD detector 的 dark count、afterpulsing、crosstalk 等噪声项。',
        userQuestionRelation: '用户问两篇如何搭配读，这条文内参考把算法论文连接回探测器噪声来源。',
        raw: '[6] SPAD detector noise modeling for single-photon imaging.',
        title: 'SPAD detector noise modeling for single-photon imaging',
        authors: 'Single-photon imaging community',
        venue: 'Single-photon detector literature',
        year: '2020',
        blockId: 'pidl-refs-spad-noise',
        anchorId: 'pidl-refs-spad-noise-a',
      }),
    ],
    refs: [
      refHit({
  ...
  makeCase(13, {
    ...
    answerMarkdown: [
      ...
      '',
      '如果你关心真实系统里能不能实时跑，还可以把 robust real-time SPI supplement 当作非深度学习/系统鲁棒性对照。[3](#qa-piln-position-a3) 文内上游网络参考 [R18](#qa-piln-position-b18) 可以帮助继续追 image-loop network 的来源。',
    ].join('\n'),
    citeDetails: [
      systemA({
        num: 1,
      ...
      systemB({
        num: 18,
        anchor: 'qa-piln-position-b18',
        citingDoc: piln,
        headingPath: 'PILN / References',
        context: 'PILN cites upstream deep learning reconstruction work for single-pixel imaging.',
        answerClaim: 'PILN 属于深度学习 reconstruction 方法的延伸，而不是独立于 DL-SPI 主线。',
        upstreamWorkRole: '上游网络方法提供了 image reconstruction architecture 和 training strategy 的来源。',
        userQuestionRelation: '用户问 PILN 和综述主线的关系，这条文内参考把具体网络放回 DL-SPI 脉络。',
        raw: '[18] Deep learning reconstruction for single-pixel imaging.',
        title: 'Deep learning reconstruction for single-pixel imaging',
        authors: 'Computational imaging community',
        venue: 'Optics and machine learning literature',
        year: '2021',
        blockId: 'piln-refs-dl-spi',
        anchorId: 'piln-refs-dl-spi-a',
      }),
    ],
    refs: [
      refHit({
