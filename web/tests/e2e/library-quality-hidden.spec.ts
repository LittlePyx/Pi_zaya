...
test('library hides internal quality maintenance from ordinary users by default', async ({ page }) => {
  ...
  await expect(page.getByTestId('library-file-quality-chip')).toHaveAttribute('data-quality-status', 'warning')
  await expect(page.getByTestId('library-file-source-readiness')).toHaveAttribute('data-source-readiness', 'blocked')
