...
test('research QA acceptance: polished refs and both citation systems stay clickable', async ({ page }) => {
  ...
  await expect(page.getByTestId('citation-popover-system-a-claim')).toContainText('低采样测量下改善单像素成像')
  ...
  await expect(page.getByTestId('citation-popover-system-a-support')).toContainText('端到端编码器/解码器')
  ...
  await expect(page.getByTestId('citation-popover-system-b-claim')).toContainText('不是 SCINeRF 作者新发明')
  await expect(page.getByTestId('citation-popover-system-b-context')).toContainText('ADMM-based optimization')
  await expect(page.getByTestId('citation-popover-system-b-role')).toContainText('通用优化框架')
  await expect(page.getByTestId('citation-popover-system-b-relation')).toContainText('上游方法背景')
