...
test('render packet contract can drive body render and strict locate without top-level render fields', async ({ page }) => {
  ...
  await expect(page.getByTestId('citation-popover-explain')).toContainText('当前论文在这里引用了哪篇上游工作')
  await expect(page.getByTestId('citation-popover-flow')).toContainText('当前论文引用处')
  await expect(page.getByTestId('citation-popover-system-b-claim')).toContainText('答案里的这句话')
  await expect(page.getByTestId('citation-popover-system-b-context')).toContainText('当前论文在哪里提到它')
  await expect(page.getByTestId('citation-popover-system-b-role')).toContainText('这篇上游文献提供了什么')
  await expect(page.getByTestId('citation-popover-system-b-relation')).toContainText('它和你的问题有什么关系')
  await expect(page.getByTestId('citation-popover-system-b-citing-source')).toContainText('Fixture Paper')
  await expect(page.getByTestId('citation-popover-system-b-cited-source')).toContainText('Single-shot compressive spectral imaging')
...
test('system A citation popover shows source location, evidence quote, and opens strict reader target', async ({ page }) => {
  ...
  await expect(page.getByTestId('citation-popover-binding-status')).toContainText('NeRF')
  await expect(page.getByTestId('citation-popover-binding-status')).toContainText('rays')
  await expect(popover).toContainText('答案依据')
  await expect(page.getByTestId('citation-popover-system-a-source')).toContainText('Fixture Paper')
  await expect(page.getByTestId('citation-popover-system-a-anchor-kind')).toContainText('句子')
  await expect(popover).toContainText('定位到原文位置')
  await expect(popover).toContainText('Fixture Paper / 2. Method')
  await expect(popover).toContainText('原文怎么说')
  await expect(popover).toContainText('Given a set of input multi-view images')
  await expect(popover).toContainText('为什么能支撑这句话')
...
test('plain numeric citations become clickable from refs hits when cite details are absent', async ({ page }) => {
  ...
  await expect(page.getByTestId('citation-popover-system-a-source')).toContainText('Deep Learning SPI Review')
  ...
  await citeChips.nth(1).click()
  await expect(page.getByTestId('citation-popover-system-a-source')).toContainText('PILN Paper')
