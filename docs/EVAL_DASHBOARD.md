...
| Answer runtime self-check | Semi-automated | Inspect stored `message.meta.answer_runtime_check` or stream `answer_runtime_check` for profile/source-summary/notice-shape/clutter failures. The check is meta-only and should not be rendered in the main answer. | TBD |
