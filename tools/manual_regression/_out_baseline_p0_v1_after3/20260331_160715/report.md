...
```text
The authors claim a space-bandwidth-time product (SBP-T) of **41,667 pixels/s**, which is about **3× higher** than the largest previously reported value in single-pixel holography (SPH) literature [[CITE:s11b51525:26]]. They emphasize that this throughput was achieved using a **single-pixel detector**, and further note that reconstructed images can reach $256 \times 256 = 65{,}536$ pixels—about 4× larger than prior SPH reports.

They frame this high throughput as critical for biological tissue imaging because it enables large-field-of-view (e.g., $14.9\,\text{mm} \times 11.1\,\text{mm}$) monitoring while maintaining rich spatial information, and because single-pixel detectors offer advantages (low dark noise, high sensitivity, broad spectral compatibility, low cost) over pixel-array detectors—especially at wavelengths where conventional cameras are unavailable or prohibitively expensive.
```
