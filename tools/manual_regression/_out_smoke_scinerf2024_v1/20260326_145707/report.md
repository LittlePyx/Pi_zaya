...
| SCINERF2024_CITATION_ADMM | cvpr_2024_scinerf | citation_lookup | FAIL | PASS | PASS | FAIL | n/a | n/a | n/a |
| SCINERF2024_METHOD_IMPL_ITER_BATCH | cvpr_2024_scinerf | method | FAIL | FAIL | n/a | FAIL | n/a | n/a | n/a |
...
```text
The paper states this explicitly in SCINeRF: Neural Radiance Fields from a Snapshot Compressive Image:
> Yunhao Li$^{1,2}$ Xiaodong Wang$^{1,2}$ Ping Wang$^{1,2}$ Xin Yuan$^{2}$ Peidong Liu$^{2\dagger}$ $^{1}$Zhejiang University $^{2}$Westlake University $\{$liyunhao, wangxiaodong, wangping, xyuan, liupeidong$\}$@westlake.edu.cn
```
