...
def _message_cite_ref_nums(message: dict[str, Any]) -> list[int]:
    nums: list[int] = []
    for item in list(message.get("cite_details") or []):
...
def _evaluate_citation_gate(
    ...
    matched_ref_nums = sorted({num for num in rendered_ref_nums + resolved_ref_nums if num > 0})
...
def _evaluate_jump_gate(answer_text: str, provenance: dict[str, Any], rules: dict[str, Any]) -> dict[str, Any]:
    reasons: list[str] = []
...
def _evaluate_case(
    ...
    *,
    ...
    if isinstance(checks.get("locate"), dict):
        gate_results["locate"] = _evaluate_locate_like_gate(provenance, dict(checks.get("locate") or {}), gate_name="locate")
    ...
    if isinstance(checks.get("jump"), dict):
        gate_results["jump"] = _evaluate_jump_gate(answer, provenance, dict(checks.get("jump") or {}))
...
def _render_report(
    *,
    ...
    lines.append(f"- overall: **{'PASS' if totals['fail'] == 0 else 'FAIL'}**")
    lines.append("")
    lines.append("## Cases")
    lines.append("")
    lines.append("| Case | Paper | Family | Status | answer | cite | locate | section | panel | jump |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|")
    for item in results:
        lines.append(
