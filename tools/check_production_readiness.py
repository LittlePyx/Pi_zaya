...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--token",
        default=os.environ.get("KB_ACCESS_TOKEN") or os.environ.get("KB_API_TOKEN") or os.environ.get("KB_AUTH_TOKEN") or "",
        help="Access token for protected /api/readiness. Defaults to KB_ACCESS_TOKEN/KB_API_TOKEN/KB_AUTH_TOKEN.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        payload = request_json(readiness_url, token=args.token, timeout_s=float(args.timeout))
        ...
        if exc.code == 401:
            print("Hint: pass --token or set KB_ACCESS_TOKEN.", file=sys.stderr)
        return 2
