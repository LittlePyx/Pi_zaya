...
def test_run_benchmark_suite_pairs_profiles_and_reverses_even_repeats(tmp_path, monkeypatch):
    ...
    profiles = [
        ...
        BenchmarkProfile(name="candidate", speed_mode="normal", stage_timings=True),
    ]
...
def test_compare_paired_profile_runs_reports_structural_quality_regression(tmp_path):
    base_dir = tmp_path / "base"
    candidate_dir = tmp_path / "candidate"
    base_dir.mkdir()
    candidate_dir.mkdir()
    (base_dir / "output.md").write_text(
        "# Paper\n\n![Figure](./assets/fig_1.png)\n\n$$x=1$$\n",
        encoding="utf-8",
    )
    (candidate_dir / "output.md").write_text("# Paper\n\n$$x=1$$\n", encoding="utf-8")
    runs = [
        {
            "profile": "control",
            "pdf_name": "paper.pdf",
    ...
    rows = benchmark_module.compare_paired_profile_runs(
        runs,
        [
            BenchmarkProfile(name="control", speed_mode="normal"),
            BenchmarkProfile(name="candidate", speed_mode="normal"),
        ],
    )

    assert rows[0]["elapsed_improvement_pct"] == 20.0
    assert rows[0]["quality_comparison_available"] is True
    assert rows[0]["quality_gate_passed"] is False
    assert rows[0]["regression_flags"] == ["images_dropped"]
